...
//level 1
function getCyclesDays() {
    return __awaiter(this, void 0, void 0, function* () {
        //CYCLES TO DAYS OBJECT CONSTRUCTION
        const cycleDocs = yield CycleModel.find().sort({ cycleNumber: 1 });
...
//level 1
function getBakerHistory(address) {
    return __awaiter(this, void 0, void 0, function* () {
...
//level 1
function getPricesAndMarketCap(fiat) {
    return __awaiter(this, void 0, void 0, function* () {
        //PRICE AND MARKET CAPITAL OBJECT
        let price = `price${fiat}`;
        let marketCap = `marketCap${fiat}`;
        let priceAndMarketCapData = yield BlockchainModel.find();
        let finalData = [];
...
function getRewards(address) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const cycles = yield getCyclesDays();
        ...
        for (i = 0, j = urlSet.length; i < j; i += chunk) {
            ...
            result = yield axios.all(temporary.map(url => axios.get(url)));
            // do whatever
            results.push(...result);
        }
        ...
        const response2 = yield axios.get(url2);
        let objectArray = [];
        let object = {};
...
function getTransactions(address) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const rewardFetch = yield getBakerHistory(address);
        //transaction tzkt url - https://api.tzkt.io/v1/operations/transactions?anyof.sender.target={$address} will return operations where sender OR target is equal to the specified value. This parameter is useful when you need to retrieve all transactions associated with a specified account.
        let url2 = `https://api.tzkt.io/v1/operations/transactions?anyof.sender.target=${address}`;
        const response2 = yield axios.get(url2);
        let objectArray = [];
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (i = 0; i < iter2length; i++) {
            try {
                ...
                if (q > realizedQuantity && realizedQuantity != 0) {
                    ...
                    unrealObj = {
                        date: unrealizedBasisRewards[0].date,
                        basisReward: unrealizedBasisRewards[0].basisReward -
                    ...
                    unrealObj = {
                        date: unrealizedBasisRewardsDep[0].date,
                        rewBasisDepletion: unrealizedBasisRewardsDep[0].rewBasisDepletion -
                    ...
                    unrealObj = {
                        date: unrealizedBasisRewardsMVDep[0].date,
                        rewBasisMVDepletion: unrealizedBasisRewardsMVDep[0].rewBasisMVDepletion -
                            realizedQuantity * basisPrice, // + dep
                    };
                    unrealizedBasisRewardsMVDep.shift();
        ...
        */
        ...
        realizedObj = {
            ...
            realizedRewards: foundRealizeHistory.realizedRewards,
 ...
 *
    ...
    */
//level 3
function analysis(address, basisDate, fiat) {
    return __awaiter(this, void 0, void 0, function* () {
        //label objects by blocks, delete repeats, remove clutter
        let pricesForUser = yield getPricesAndMarketCap(fiat);
        ...
        //DATA DEPENDCEIES
        //ADD tran
        var values = yield getRewards(address);
        ...
        let basisBalances = yield getBalances(address);
        ...
        let supply = [];
        let mvdAnal = [];
        const supplyDocs = yield StatisticModel.find();
        ...
        for (i = 0; i < rewards.length && i < mvdAnal.length; i++) {
            if (i > 0 && i < basisRewards.length - 1) {
                ...
                for (j = 0; j < tranArray.length; j++) {
                    try {
                        if (formatDate(tranArray[j].date) ==
                            formatDate(date)) {
                            tranVal = tranArray[j].amounnt;
                        }
                        else if (formatDate(tranArray[j].date) >
                            formatDate(date)) {
                            if (formatDate(tranArray[j].date) <
                                formatDate(nextDate)) {
                                tranVal = tranArray[j].amounnt;
...
*/
//REALIZE OBJECTS
function saveRealize(realizing_obj) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (i = 0; i < realizing_obj.realizingRewards.length; i++) {
            ...
            savedObject.realizedBasisRewards.push(realizing_obj.realizingBasisRewards[i]);
            savedObject.realizedBasisRewardsDep.push(realizing_obj.realizingBasisRewardsDep[i]);
            savedObject.realizedBasisRewardsMVDep.push(realizing_obj.realizingBasisRewardsMVDep[i]);
        }
...
function autoAnalysis(address, fiat) {
    return __awaiter(this, void 0, void 0, function* () {
        //label objects by blocks, delete repeats, remove clutter
        //DATA DEPENDCEIES
        //ADD tran
        ...
        for (i = 0; i < positiveTrans.length; i++) {
            let date = yield formatDate(positiveTrans[i].date);
        ...
        for (i = 0; i < netPositives.length; i++) {
            let date = yield formatDate(netPositives[i].date);
            for (j = 0; j < pricesForUser.length; j++) {
                let priceDate = yield formatDate(pricesForUser[j].date);
                if (priceDate == date) {
...
function avgBasisPrice(address, fiat) {
    return __awaiter(this, void 0, void 0, function* () {
        let transObject = yield getTransactions(address);
        let balanceObject = yield getBalances(address); //balances are EOD
        let priceObject = yield getPricesAndMarketCap(fiat);
        //POSTIVE TRANSACTIONS OBJECT
        let positiveTrans = [];
        ...
        for (i = 0; i < netPositives.length; i++) {
            ...
            for (j = 0; j < priceObject.length; j++) {
                let priceDate = yield formatDate(priceObject[j].date);
                if (priceDate == date) {
