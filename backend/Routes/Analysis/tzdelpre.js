...
async function realizeRew(realizedQuantity){
    //take realize post api
    // let realizedQuantity = 900

    //get the realizehistoryobject from the db
    let foundRealizeHistory = await RealizeSet.find()
    ...
    let basisPrice = foundRealizeHistory[0].basisPrice
    let unrealrewards = []
    for(i = 0; i < foundRealizeHistory[0].unrealizedBasisRewards.length; i++){
        rewardQ = foundRealizeHistory[0].unrealizedBasisRewards[i].basisReward / basisPrice
        date = foundRealizeHistory[0].unrealizedBasisRewards[i].date
        reward = {
            "date": date,
            "q": rewardQ
        }
    ...
    unrealizedBasisRewards = foundRealizeHistory[0].unrealizedBasisRewards
    unrealizedBasisRewardsDep = foundRealizeHistory[0].unrealizedBasisRewardsDep
    unrealizedBasisRewardsMVDep = foundRealizeHistory[0].unrealizedBasisRewardsMVdep
    ...
    for (i = 0; i < unrealrewards.length; i++){
        ...
        //technically i would need to recalculate the depletion to make this perfect using bv? or by scaling with proportions of proportions
            ...
            dep = unrealizedBasisRewardsDep[i-1].rewBasisDepletion - (unrealrewards[i-1].q * basisPrice)
            newrealizngObj = {
            ...
            unrealObj = {
                "date": unrealrewards[i-1].date,
                "q": unrealrewards[i-1].q - realizedQuantity
            }
            unrealrewards.shift()
    ...
    for (i = 0; i < unrealrewards.length; i++){
        unrealizedRewardAgg += unrealrewards[i].q
    ...
    */
    ...
    realizedObj = {
        ...
        "address": foundRealizeHistory[0].address,
        "basisDate": foundRealizeHistory[0].basisDate,
        "basisPrice": foundRealizeHistory[0].basisPrice,
        "fiat": foundRealizeHistory[0].fiat,
        "realizedRewards" : foundRealizeHistory[0].realizedRewards, //again not working ~ unrealized rewards and realized rewards from .find()
        "realizedBasisRewards" : foundRealizeHistory[0].realizedBasisRewards,
        "realizedBasisRewardsDep" : foundRealizeHistory[0].realizedBasisRewardsDep,
        "realizedBasisRewardsMVdep" : foundRealizeHistory[0].realizedBasisRewardsMVdep,
