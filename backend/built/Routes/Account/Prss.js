...
router.post("/changepw", function (req, res) {
    ...
    async.waterfall([
        function (cb) {
            ...
            if (vld.hasFields(req.body, ["email", "password", "new_password"], cb))
                if (req.body.password.startsWith("temp_")) {
                    Temp_Pw.find({ email: req.body.email }, function (err, docs) {
                        if (err)
                            cb(err);
                        cb(null, docs);
                    });
        ...
        function (pw_hash, cb) {
            User.findOneAndUpdate({ email: req.body.email }, { $set: { password: pw_hash } }, function (err, docs) {
                if (err)
                    cb(err);
                cb(null, docs);
            });
...
router.post("/forgotpw", function (req, res) {
    ...
    async.waterfall([
        ...
        function (result, cb) {
            if (vld.check(result.length < 1, "Too many forgot password requests", null, cb))
                bcrypt.genSalt(10, cb);
        },
        function (salt, cb) {
            // create temp pwr
    ...
    async.waterfall([
        ...
        function (prsArr, cb) {
            if (vld.check(prsArr.length, Tags.notFound, null, cb)) {
                ...
                let userCopy = Object.assign({}, prsArr[0])._doc;
...
router.post("/", function (req, res) {
    ...
    async.waterfall([
        ...
        function (hashedpass, cb) {
            const { firstName, lastName, email, password, termsAccepted, role, } = body;
            user = new User({
                firstName,
                lastName,
                email,
